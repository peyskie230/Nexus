...
export function AppLayout({ profile, children }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-14 left-0 right-0 z-50 bg-slate-900 border-b border-slate-800 px-4 py-3 space-y-1">
          {navItems.map(({ href, icon: Icon, label }) => {
            const isActive = pathname === href || pathname.startsWith(href + '/')
            return (
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-all">
            <LogOut className="w-4 h-4" />
            Log out
          </button>
        </div>
